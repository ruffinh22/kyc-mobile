...
async function exporterReporting(quoi){
  ...
  try{
    ...
    else if(quoi==='donnees_heures') await exportDonneesHeures(debut,fin);
...
async function exportDonneesHeures(debut,fin){
  // Parcourir chaque jour de la periode et appeler /api/dossiers/par-heure
  const jours=[];
  // Construction des dates sans decalage de fuseau (format YYYY-MM-DD local)
  const fmt=(dt)=>dt.getFullYear()+'-'+String(dt.getMonth()+1).padStart(2,'0')+'-'+String(dt.getDate()).padStart(2,'0');
  let d0=new Date(debut+'T12:00:00'); const d1=new Date(fin+'T12:00:00');
  while(d0<=d1){ jours.push(fmt(d0)); d0.setDate(d0.getDate()+1); }
  const tranche=[['Date','Heure','Total','0-2 min %','2-5 min %','5-10 min %','10-15 min %','15-30 min %','>30 min %']];
  const wp=[['Date','Heure','Total','Waiting time (min)','Processing time (min)']];
  let n=0;
  for(const j of jours){
    try{
      const r=await fetch('/api/dossiers/par-heure?date='+j,{headers:{'Authorization':'Bearer '+repToken()}});
      if(!r.ok) continue;
      const data=await r.json();
      (data.lignes||[]).forEach(l=>{
        tranche.push([j,l.heure,l.total,l.pct_0_2+'%',l.pct_2_5+'%',l.pct_5_10+'%',l.pct_10_15+'%',l.pct_15_30+'%',l.pct_30p+'%']);
        wp.push([j,l.heure,l.total,l.waiting_moy,l.processing_moy]);
        n++;
      });
    }catch(e){}
  }
  if(n===0){ repToast('Aucune donnee par heure sur la periode','err'); return; }
  const wb=XLSX.utils.book_new();
  const ws1=XLSX.utils.aoa_to_sheet(tranche);
  ws1['!cols']=[{wch:12},{wch:8},{wch:8},{wch:11},{wch:11},{wch:11},{wch:12},{wch:12},{wch:11}];
  XLSX.utils.book_append_sheet(wb,ws1,'Repartition par tranche');
  const ws2=XLSX.utils.aoa_to_sheet(wp);
  ws2['!cols']=[{wch:12},{wch:8},{wch:8},{wch:20},{wch:22}];
  XLSX.utils.book_append_sheet(wb,ws2,'Waiting-Processing');
  XLSX.writeFile(wb,'donnees_par_heure_'+debut+'_au_'+fin+'.xlsx');
  repToast('✅ Rapport exporté ('+n+' lignes)','ok');
}
