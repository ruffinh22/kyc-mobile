...
async function main() {
  ...
  if (!matricule || !nom || !password) {
    console.error('Usage: npx ts-node src/scripts/create-superviseur.ts MATRICULE Nom [Prenom] MotDePasse');
    process.exit(1);
  }
  ...
  console.log(`✅ Superviseur créé : id=${id} matricule=${matricule.toUpperCase()}`);
  process.exit(0);
}
