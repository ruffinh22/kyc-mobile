      ...
      <View style={s.rowBetween}>
        <View style={s.left}>
          ...
          <View style={s.metaRow}>
            ...
            <Text style={s.duration}>Durée : {formatDuration(item.durationSec) || '00:00'}</Text>
          </View>
        </View>
        <View style={[s.badge, statusBadgeStyle(item.status)]}>
          <Text style={[s.badgeText, { color: statusTextColor(item.status) }]}>{statusLabel(item.status)}</Text>
        </View>
      </View>
      <Text style={s.uuid}>UUID: {item.callUuid}</Text>
    </View>
  );
...
const s = StyleSheet.create({
  ...
  list: { flexGrow: 1, paddingHorizontal: 20, paddingVertical: 16, paddingBottom: 120, gap: 12 },
  ...
  emptyText: { marginTop: 8, color: C.ink3, textAlign: 'center', fontSize: T.sm },
  card: {
    backgroundColor: 'rgba(255,255,255,0.96)', borderRadius: R.xl, padding: 14, borderWidth: 1, borderColor: 'rgba(15,23,42,0.06)',
    shadowColor: '#0F1720', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 5 }, elevation: 2,
  },
  ...
  duration: { color: C.ink2, fontSize: T.xs, fontWeight: '700' },
  ...
  uuid: { marginTop: 10, color: C.ink3, fontSize: T.xs },
  ...
  fab: {
    ...
    shadowColor: C.shadowYellow, shadowOpacity: 0.28, shadowRadius: 12, shadowOffset: { width: 0, height: 6 },
    elevation: 8,
  },
